...
void Rasterization::initialize(int shader_)
{
...
void Rasterization::rasterize(Shader *shader)
{
    ...
    Vector2d *pixel = new Vector2d[mesh.num_vertex];
    double *depth = new double[mesh.num_vertex];
    Vector3d *vertex_normal = new Vector3d[mesh.num_vertex]; // normal vector of vertex
    int count = 0;
    ...
    for (int i = 0; i < mesh.num_vertex; i++)
    {
        ...
        back_projection(mesh.vertices[i], pixel[i]);
    }
...
//                    double current_depth = get_depth(triangle, pixel_center);
                    ...
                    if (current_depth < z_buffer[i * x + j])
                    {
                        ...
                        {
                            Vector3d normal_0 = vertex_normal[triangle.vertex_0];
                            Vector3d normal_1 = vertex_normal[triangle.vertex_1];
                            Vector3d normal_2 = vertex_normal[triangle.vertex_2];
                            //                            Vector3d normal = u * normal_0 + v * normal_1 + w * normal_2;
...
//    double n = (P.dot(triangle.normal) + triangle.D) / pc.dot(triangle.normal);
//    cross = -(P + n * pc);
};

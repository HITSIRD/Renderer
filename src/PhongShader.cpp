...
float PhongShader::shadow(Light *light, const Fragment &frag) {
    if (light->type == POINT) {
        ...
        auto *sun = (SunLight *) light;
        if (sun->shadowMap) {
            float4 dir_light = (light->position - frag.world).normalized();
            float light_normal_cos = abs(frag.normal.dot(dir_light));
            //            if (light_normal_cos <= 0)
            //            {
            //                return 0;
            //            }
            float4 shadowMapIndex = sun->matrixWorldToScreen * frag.world;
            float bias = shadowMapIndex.z() * 0.001f / light_normal_cos;
            int index_x = (int) shadowMapIndex.x();
            int index_y = (int) shadowMapIndex.y();
